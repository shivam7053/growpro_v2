...
// ============================================
// app/api/send-purchase-confirmation/route.ts
...
export async function POST(req: NextRequest) {
  try {
    const {
      ...
      masterclassId,
      videoId,
      purchaseType, // "masterclass" | "video" | "upcoming_registration"
    } = await req.json();
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `${isPaid ? '💳' : '✅'} ${isVideo ? 'Video' : 'Masterclass'} ${isPaid ? 'Purchase' : 'Enrollment'} Confirmed - ${isVideo ? videoTitle : masterclassTitle}`,
      html: htmlContent,
    });
