...
/**
 * ✅ UPDATED: parse Firestore doc into Masterclass object
 * Includes new Zoom + masterclass_source functionality
 */
export const parseMasterclassData = (
  docId: string,
  data: Record<string, any>
  ...
  // ⭐ NEW FIELD: masterclass_source (default = "youtube")
  const masterclass_source: MasterclassSource =
    data.masterclass_source === "zoom"
      ? "zoom"
      : "youtube";

  // ⭐ NEW Zoom fields
  const zoom_start_time =
    data.zoom_start_time instanceof Timestamp
      ? data.zoom_start_time.toDate().toISOString()
      : typeof data.zoom_start_time === "string"
      ? data.zoom_start_time
      : "";

  const zoom_end_time =
    data.zoom_end_time instanceof Timestamp
      ? data.zoom_end_time.toDate().toISOString()
      : typeof data.zoom_end_time === "string"
      ? data.zoom_end_time
      : "";
  ...
  return {
    ...
    thumbnail_url: String(data.thumbnail_url ?? ""),
    description: String(data.description ?? ""),
    type: type as Masterclass["type"],

    // ⭐ New fields
    masterclass_source,
    zoom_link: String(data.zoom_link ?? ""),
    zoom_meeting_id: String(data.zoom_meeting_id ?? ""),
    zoom_passcode: String(data.zoom_passcode ?? ""),
    zoom_start_time,
    zoom_end_time,
...
/**
 * Filters masterclasses based on search + type + enrollment
 */
export const filterMasterclasses = (
  masterclasses: Masterclass[],
  searchQuery: string,
  ...
  if (searchQuery.trim()) {
    const query = searchQuery.toLowerCase();
    filtered = filtered.filter((mc) =>
      [mc.title, mc.speaker_name, mc.speaker_designation, mc.description].some(
  ...
  switch (filterType) {
    case "free":
      filtered = filtered.filter(
        (mc) => mc.type === "free" || mc.starting_price === 0
      );
      break;
    case "paid":
      filtered = filtered.filter(
        (mc) => mc.type === "paid" || mc.starting_price > 0
      );
      break;
    case "featured":
      filtered = filtered.filter((mc) => mc.type === "featured");
...
/**
 * Masterclass access evaluator
 */
export function isMasterclassAccessible(
  masterclass: Masterclass,
  userId?: string
): boolean {
  if (masterclass.type === "upcoming") return false;

  if (userId && masterclass.joined_users?.includes(userId)) return true;
...
/**
 * Button text depending on user status
 */
export function getMasterclassActionText(
  masterclass: Masterclass,
  userId?: string
