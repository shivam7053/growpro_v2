...
export default function UpcomingMasterclassSection({
  ...
  const displayedMasterclasses = upcomingMasterclasses.slice(0, 4);

  if (!loading && upcomingMasterclasses.length === 0) return null;

  return (
    <motion.section
      className="py-20 bg-gray-100 dark:bg-gray-900 transition-colors duration-300 relative z-10"
      initial="initial"
      whileInView="animate"
      viewport={{ once: true }}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          <div className="text-center md:text-left">
            ...
            <p className="text-gray-700 dark:text-gray-300 text-lg">
              Be the first to access newly scheduled sessions
            </p>
          </div>

          {/* FIXED BUTTON → Hover works correctly */}
          <Link
        ...
        {loading && (
          <div className="flex justify-center py-14">
            <div className="animate-spin h-12 w-12 rounded-full border-4 border-gray-300 border-t-black dark:border-gray-700 dark:border-t-white"></div>
          </div>
        )}
        ...
        {!loading && displayedMasterclasses.length === 0 && (
          ...
          >
            ...
            <p className="font-medium">No upcoming masterclasses right now.</p>
          </motion.div>
        )}

        {/* 4 MOST RECENT MASTERCLASSES */}
        {!loading && displayedMasterclasses.length > 0 && (
          <motion.div
            variants={fadeInUp}
