...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const recipients = userDocs
      .map(doc => doc.data())
      .filter(ud => ud && ud.email) as { full_name?: string, email: string }[];
    ...
    // Send email to all recipients
    for (const recipient of recipients) {
      const html = generateUpdateEmail(recipient.full_name || "there", after, newContent);
      const subject = `🚀 New Content Added to ${after.title}!`;
      const encodedSubject = encodeSubject(subject);
      await sendEmail(recipient.email, encodedSubject, html);
      await new Promise(r => setTimeout(r, 700)); // Rate limit
...
function generateUpdateEmail(userName: string, masterclass: Masterclass, newContent: MasterclassContent[]): string {
  const newContentHtml = newContent.map(item => {
    // Get content type with proper type checking
    let contentType = 'Content';
    let scheduledInfo = '';
    
    if (isYoutubeContent(item)) {
      contentType = 'YouTube Video';
      if (item.scheduled_date) {
        scheduledInfo = `<p style="margin:5px 0 0; font-size: 0.9em; color: #555;">Scheduled: ${new Date(item.scheduled_date).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}</p>`;
      }
    } else if (isZoomContent(item)) {
      contentType = 'Live Zoom Session';
      if (item.scheduled_date) {
        scheduledInfo = `<p style="margin:5px 0 0; font-size: 0.9em; color: #555;">Scheduled: ${new Date(item.scheduled_date).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}</p>`;
      }
    } else if (isTestContent(item)) {
      contentType = 'Assessment Test';
      scheduledInfo = `<p style="margin:5px 0 0; font-size: 0.9em; color: #555;">Passing Grade: ${item.passingGrade}%</p>`;
    }
